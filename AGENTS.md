...
Qwen notes generation should keep reasoning enabled, but the app must consume each backend's reasoning channel separately from final content. Run bundled `llama-server` with `--reasoning on --reasoning-format deepseek` so reasoning lands in `message.reasoning_content` / streamed reasoning deltas instead of `message.content`, send Ollama chat requests with `think=true`, ignore provider reasoning fields when final content is present, and treat reasoning-only payloads (`message.reasoning_content` or `message.thinking` without a final answer) as explicit runtime errors instead of empty notes.

Bundled `llama-server` supports native reasoning throttling, so notes runs can cap chain-of-thought before it eats the whole output budget. Derive `--reasoning-budget` from the largest planned prompt output budget (currently 75% of max output tokens) and set `--reasoning-budget-message` to force a final answer when the budget is exhausted; keep Ollama unchanged because the `/api/chat` path used here exposes `think` but not an equivalent numeric reasoning-budget control.
