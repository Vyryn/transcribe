...
Qwen notes generation should keep reasoning enabled, but the app must consume each backend's reasoning format correctly. Run bundled `llama-server` with `--reasoning on --reasoning-format none` so `<think>` traces stay inline and can be stripped from saved output, send Ollama chat requests with `think=true`, and treat backend-specific reasoning-only payloads (`message.reasoning_content` or `message.thinking` without a final answer) as explicit runtime errors instead of empty notes.
